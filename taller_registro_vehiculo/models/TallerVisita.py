...
class TallerVisita(models.Model):
    ...
    # Esto le da a tu modelo la capacidad de tener chatter, seguidores y actividades.
    _name = 'taller.visita'
    ...
    ], string='Estado', default='pendiente', tracking=True) # tracking=True para que los cambios de estado aparezcan en el chatter
    tipo_trabajo = fields.Selection(, string='Tipo de Trabajo', required=True)
    ...
    # --- CAMBIO 2: Sobrescribir el método 'create' para crear la actividad ---
    @api.model_create_multi
    def create(self, vals_list):
        # Primero, creamos las visitas como siempre, llamando al método original.
        visitas = super().create(vals_list)
        
        # Buscamos el grupo de seguridad que crearemos en el siguiente paso.
        ...
        if notification_group:
            # Obtenemos todos los usuarios que pertenecen a ese grupo.
            users_to_notify = notification_group.users
            
            for visita in visitas:
                # Solo creamos la actividad si la visita está en estado 'pendiente'.
                if visita.estado == 'pendiente':
                    # Preparamos los datos para la actividad.
                    activity_data = {
                        ...
                        'date_deadline': visita.fecha, # La fecha límite es la fecha de la visita.
                    }
                    
                    # Creamos una actividad para cada usuario del grupo.
